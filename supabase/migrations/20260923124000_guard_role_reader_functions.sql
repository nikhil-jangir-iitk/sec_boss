...
-- `anon` does not keep it: 20251023000014 granted it EXECUTE on all three, and
-- the enforce_explicit_anon_grants event trigger (20260908000000, 20260912120000)
-- revokes PUBLIC and `anon` from a public routine on every CREATE FUNCTION,
-- CREATE OR REPLACE included. So re-issuing these three also takes them away
-- from the anon key, which is wanted, and the tests pin it rather than leave it
-- to that side effect.
--
...
ALTER FUNCTION "public"."can_read_user_roles"("uuid") SECURITY DEFINER;
