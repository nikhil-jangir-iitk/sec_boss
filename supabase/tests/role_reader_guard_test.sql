...
--
-- Ordered on purpose. The first block is the leak, and it is written so that it
-- FAILS against a database without this migration: an ordinary user asks for
-- another user's roles and gets them. Everything after it is what must keep
-- working, and the helper-specific checks come last because the helper does not
...
-- And their own roles still come back, which is RoleService's self path.
select isnt(
